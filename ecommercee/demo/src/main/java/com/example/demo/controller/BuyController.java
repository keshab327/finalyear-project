	...
	//foe one payment only
		...
		try {
			...
			Productbuy buy=productbuyrepo.findById(productbuyid).get();
			 ...
			 //setadress
	   order.setProvience(payment.getProvience());
	   order.setDistrict(payment.getDistrict());
	   order.setWardno(payment.getWardno());
	   
	   
	   order.setShippingdistrict(payment.getShippingdistrict());
	   order.setShippingprovience(payment.getShippingprovience());
	   order.setShippingwardno(payment.getShippingwardno());
	   order.setPhoneshipping(shippingphoneno);
			...
			order.setAdminsendorder("NOTSENT");
			order.setCancellcheck(false);
			...
			model.addAttribute("provience",payment.getProvience());
			model.addAttribute("district",payment.getDistrict());
			model.addAttribute("wardno",payment.getWardno());
			
			model.addAttribute("shippingdistrict",payment.getShippingdistrict());
			model.addAttribute("shippingprovience",payment.getShippingprovience());
			model.addAttribute("shippingwardno",payment.getShippingwardno());
			
			if(shippingphoneno!=0) {
